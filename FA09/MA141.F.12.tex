...
\begin{center}
\begin{tikzpicture}
\draw (0.5\linewidth, 0cm) node[text justified, text
width=0.5\linewidth, draw, rounded corners] { 
We write $\displaystyle{\lim_{x\to a}} f(x) = L$ if for all
$\varepsilon > 0$ there exists $\delta >0$ such that $\lvert x - a
\rvert < \delta$ implies $\lvert f(x) - L \rvert < \varepsilon$.};   
\end{tikzpicture}
\end{center}
...
\begin{enumerate}
\item $f(x) = \pi x^{-1} (x^5 - 5 x^4 + 10 x^3 -10 x^2 + 5 x - 1)$
\vspace{4cm}
\begin{flushright}
  \begin{tikzpicture}
  ...
  \end{tikzpicture}
\end{flushright}
\item $\displaystyle{g(t) = \frac{t^2-5}{\sqrt{t}}}$
\vspace{4cm}
\begin{flushright}
  \begin{tikzpicture}
