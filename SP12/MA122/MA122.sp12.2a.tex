...
decaying exponentially at a continuous rate of 0.25\% per year.  What is the half-life of ozone in the upper atmosphere?}
\vspace{4.5cm}
\begin{flushright}
  \begin{tikzpicture}
...
\end{flushright}
\hrule
{\problem[10 pts] \em If $f(t)=t^2$ and $g(t)=t+2$, then $f\big( g(t) \big)$ is:
\begin{enumerate}
\item $t^2+2$
\item $t^2+4$
\item $t^2+2t+4$
...
\end{enumerate}
\hrule
{\problem[10 pts] \em For the function $f$ represented below, graph $f(x+2)$ in the same coordinate axes.}
\begin{center}
\begin{tikzpicture}
...
\begin{enumerate}
\item What is the total change in the size of the tumor during the first six months?
\vspace{0.5cm}
\begin{flushright}
  \begin{tikzpicture}
    \draw (0cm,-0.2cm) rectangle (5cm,1.2cm);
...
intervals on which the second derivative is positive.  Mark all that apply.}
\begin{enumerate}
...
\begin{tikzpicture}
\draw (0cm,0cm) node{[2 pts] (b) $y=\sqrt{\dfrac{1}{x^3}},\qquad y'=$};
\draw (3cm,-0.5cm) rectangle (6cm,0.5cm);
\end{tikzpicture}
\vspace{1cm}

\begin{tikzpicture}
\draw (0cm,0cm) node{[3 pts] (c) $y=\dfrac{3}{x}+\dfrac{4}{x^2},\qquad y'=$};
\draw (3.25cm,-0.5cm) rectangle (9.25cm,0.5cm);
\end{tikzpicture}
\vspace{1cm}

\begin{tikzpicture}
\draw (0cm,0cm) node{[2 pts] (d) $y=e^{3x},\qquad y'=$};
...
\begin{tikzpicture}
\draw (0cm,0cm) node{[4 pts] (f) $y=x^2e^{3x},\qquad y'=$};
\draw (3cm,-0.5cm) rectangle (7.75cm,0.5cm);
\end{tikzpicture}
\vspace{2cm}

\begin{tikzpicture}
\draw (0cm,0cm) node{[4 pts] (g) $y=\dfrac{5x^2}{x^3+1},\qquad y'=$};
