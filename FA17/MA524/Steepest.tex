...
Assume we have a quadratic function $p\colon \field{R}^d \to \field{R}$ satisfying $p(\boldsymbol{0}) = 0$.  There exist a vector $D = \big[q_1, \dotsc, q_d\big]$ and a symmetric matrix $Q = \big[ q_{jk} \big]_{j,k=1}^d$ (with $q_{jk}=q_{kj}$ for all $1\leq j,k \leq d$) so that
\begin{equation*}
...
&= \tfrac{1}{2} D Q^{-1} \transpose{D} - D Q^{-1} \transpose{D} = -\tfrac{1}{2} D Q^{-1} \transpose{D}.
\end{align*}
If $\x_n$ is a term in a sequence of steepest descent, then to compute $\x_{n+1}$ we proceed as follows:
\begin{enumerate}
	\item The direction of steepest descent at $\x_n$ is $\boldsymbol{v}_n = -\gradient{p}(\x_n) = -(x_n Q + D)$.
	\item The restriction $\varphi\colon (0,\infty) \to \field{R}$ of the quadratic function $p$ over the line through $\x_n$ in the direction $\boldsymbol{v}_n$ is given by
	\begin{equation*}
	\varphi(t) = p\big( \x_n -t \gradient{p}(\x_n) \big) = p(\x_n) + t\norm{\boldsymbol{v}_n} + \tfrac{1}{2}t^2 \quadratic{Q}(\boldsymbol{v}_n)
	\end{equation*}
\end{enumerate}
