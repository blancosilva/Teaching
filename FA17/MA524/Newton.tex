...
A session that computes this approximation.  We call \texttt{fmin\_ncf} with the function and its gradient (with the option \texttt{fprime=}), the initial guess $(2,2)$.  In the example below I have deactivated the option \texttt{retall=False}.  If we set it to \texttt{True}, we obtain the complete list of iterations obtained by the algorithm.
