...
\begin{example}\label{example:loop}
...
\end{align*}
This sequence turns into a loop: $x_{2n}=x_0$, $x_{2n+1}=-x_0$ for all $n\in \field{N}$ (see Figure \ref{figure:loop}).
\begin{figure}[ht!]
...
There are some theoretical results that aid in the search for a \emph{good} initial guess:
