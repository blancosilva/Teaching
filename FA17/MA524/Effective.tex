...
Using these principles, we are going to see two constructions based upon secant methods: the BFGS, and the DFP methods.
\subsection{The BFGS method}\index{BFGS method}
...
\begin{remark}
Unlike in the general Broyden's method, all the matrices $\boldsymbol{A}_n$ constructed in \eqref{equation:BFGS} are positive definite.  For a proof, see e.g.~\cite[Theorem 3.5.2]{peressini1988mathematics}.
\end{remark}

\begin{example}
There is an implementation of BFGS in the \scipy\ libraries of Python.  The following session illustrates how to use this method to approximate the minimum of the Rosenbrock function $\mathcal{R}_{1,1}$  with an initial guess $(x_0, y_0)=(-2,2)$
...
A session that computes this approximation.  We call \texttt{fmin\_bfgs} with the function and its gradient (with 	the option \texttt{fprime=}), the initial guess $(-2,2)$, and activate the option \texttt{retall=True} that offers us the complete list of approximations obtained by the algorithm.
...
\subsection{The DFP Method}\index{DFP method}
The Davidon-Fletcher-Powell method is similar to BFGS, with the roles of vectors $\boldsymbol{d}_n$ and $\y_n$ changed.  
To minimize $f \colon \field{R}^d \to \field{R}$, select an initial guess $\x_0$ and an initial \emph{positive definite} matrix $\boldsymbol{A}_0$.  If $\x_n$ and $\boldsymbol{A}_n$ have been computed, then 
\begin{enumerate}
...
\begin{remark}
Like in the BFGS method, all the matrices $\boldsymbol{A}_n$ constructed in \eqref{equation:BFGS} are also positive definite.
\end{remark}
