...
\begin{theorem}[Principal Minor Criteria]\label{theorem:PrincipalMinors}
...
\begin{theorem}[Eigenvalue Criteria]\label{theorem:eigenvalues}
...
To be able to classify possible extrema $\xstar$ for a properly differentiable function, we take into account the behavior of the function around $f(\xstar)$ with respect to the tangent hyperplane at the point $\big(\xstar, f(\xstar)\big)$.  Second derivatives make this process very easy.

\begin{theorem}[Coerciveness necessary optimality condition for minimization]\label{theorem:coerciveNecessaryMinima}
...
\end{equation*}
This function is twice continuously differentiable in $\field{R}^3$. Its continuity does not guarantee existence of a global minimum initially since the domain is not compact, but we may try our luck with its critical points.  Note $\gradient{f}(x,y,z) = \big[ e^{x-y} - e^{y-x} + 2xe^{x^2}, -e^{x-y}+e^{y-x}, 2x \big]$.  The only critical point is then $(0,0,0)$ (Why?). The corresponding Hessian is positive definite:
\begin{align*}
...
\end{align*}
The third principal minor is positive too: $\Delta_3 = 2\Delta_2 > 0$.  We have just proved that $\Hess{f}(x,y)$ is positive definite for all $(x,y) \in \field{R}^3$, and thus $(0,0,0)$ is a strict global minimum.
\end{example}
...
f(x,y) = e^{x-y} + e^{y-x}.
\end{equation*}
This function is also twice continuously differentiable, but no extrema is guaranteed.  Notice that all points $(x^\star, y^\star)$ satisfying $y^\star = x^\star$ are critical.  For such points, the corresponding Hessians and principal minors are given by
\begin{align*}
...
\end{align*}
The Hessian is positive definite for all points, hence proving that any point in the line $y=x$ is a global minimum of $f$.
\end{example}
...
f(x,y) = x^3-12xy+8y^3.
\end{equation*}
This is a polynomial of degree 3, hence twice continuously differentiable.  It is easy to see that this function has no global minima: 
\begin{equation*}
...
f(x,y) = x^4-4xy+y^4
\end{equation*}
This is a polynomial of degree 4, hence twice continuously differentiable.  There are three critical points: $(0,0)$, $(-1,-1)$ and $(1,1)$.  The latter two are both strict local minima (by virtue of Theorem \ref{theorem:sufficientMinima}).
\begin{align*}
\Hess{f}(-1,-1) = \Hess{f}(1,1) &= \begin{bmatrix} 12 & -4 \\ -4 & 12 \end{bmatrix}, &\Delta_1 &= 12 > 0, & \Delta_2 &= 128 > 0.
\end{align*}
We proved in Example \ref{example:CoerciveFunctionsGeneral} that $f$ is coercive.  By Theorems \ref{theorem:CoerciveFunctions} and \ref{theorem:coerciveNecessaryMinima} we have that $f(-1,-1)=f(1,1)=-2$ must be strict global minimum values.
\end{example}
