...
Let $f\colon \field{R}^d \to \field{R}$.  To prove that $f$ is differentiable at a point $\x_0 \in \field{R}^d$ we need a linear function $J(h) = \langle \boldsymbol{a}, h \rangle$ for some $\boldsymbol{a} \in \field{R}^d$.  Under the conditions of Theorem \ref{theorem:partialgivesDerivative} we may use
\begin{equation*}
...
To be able to classify extrema in a properly differentiable function, it is necessary to see the behavior of the function with respect to the tangent hyperplane at our candidates.  Second derivatives make this process very easy.  Before we proceed to the key results, we should refresh some terminology:

\begin{definition}[Hessian]\label{def:Hessian}
Given a twice-differentiable function $f\colon D \subseteq \field{R}^d \to \field{R}$, we define the \emph{Hessian} of $f$ at $\x \in D$ to be the following matrix of second partial derivatives:
\begin{equation*}
