...
\begin{definition}\label{def:linearMap}\index{Linear map}
A function $\boldsymbol{J} \colon \field{R}^{d_1} \to \field{R}^{d_2}$ is said to be a \emph{linear map} if it satisfies 
\begin{equation*}
\boldsymbol{J}(\x+\lambda\y) = \boldsymbol{J}(\x) + \lambda \boldsymbol{J}(\y) \text{ for all }\x, \y \in \field{R}^{d_1}, \lambda \in \field{R}.
\end{equation*}  
\end{definition}

\begin{remark}
For each real-valued linear map $J \colon \field{R}^d \to \field{R}$ there exists $\boldsymbol{a} \in \field{R}^d$ so that $J(\x) = \langle \boldsymbol{a} , \x \rangle$ for all $\x \in \field{R}^d$.  

The graph of a real-valued linear function can be identified with a hyperplane in $\field{R}^{d+1}$ :
\begin{align*}
\operatorname{Graph} J &= \{ (\x,y) \in \field{R}^{d+1} : y = J(\x) \} \\
...
\begin{remark}
For each linear map $J \colon \field{R}^{d_1} \to \field{R}^{d_2}$ there exists a matrix $\boldsymbol{A}$ of size $d_1 \times d_2$ so that $J(\x) = \boldsymbol{A} \cdot \transpose{\x}$.
\begin{equation*}
\begin{cases} 
