...
\item[Tangents to Level Curves] At every point $(x,y)$ in the domain of $f$, the gradient $\gradient{f}(x,y)$ is perpendicular to the level curve through $(x,y)$. \index{Level Curve}
\end{description}
\end{remark}

\begin{definition}\label{def:localminimum}\index{Minimum!local}\index{Minimum!value}
...
\begin{definition}\label{def:criticalpoint}\index{Critical Point}
...
\begin{theorem}\label{theorem:2DTforLEV}\index{Test!Second Derivative Test for Local Extreme Values}
...
\begin{example}[Rosenbrock Functions]\label{example:Rosenbrock}\index{Rosenbrock Function}
...
\begin{theorem}[Orthogonal Gradient]\label{theorem:OrthogonalGradient}\index{Gradient!Orthogonal}
