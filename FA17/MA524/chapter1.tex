...
\end{equation*}
Notice that the domain of posynomials may not be extended in general anywhere else in $\field{R}^d$.  For example, notice the obvious restrictions for the function $f\colon \mathfrak{P}_2 \to \field{R}$ given by $f(x_1,x_2) = x_1\sqrt{x_2} + \frac{\pi}{x_1}$.
\end{example}

\begin{example}[Polynomials]\label{example:polynomial}
\emph{Multi-variate Polynomials} have a similar structure.  There is no restriction on the sign of the coefficients $c_j$, but the exponents $\alpha_{jk}$ can only be chosen among non-negative integers.  The domain of polynomials is then easily extended to the whole space $\field{R}^d$.
\end{example}
...
Under the same conditions as in example \ref{example:MatrixMultiplication}, we construct functions $\bilinear{A} \colon \field{R}^d \times \field{R}^d \to \field{R}$ given by
\begin{equation*}
...
\end{tikzpicture}
\end{center}
Prove that a symmetric matrix $\boldsymbol{A}$ is:
\begin{enumerate}
...
Given a general square $d \times d$ matrix $\boldsymbol{A}$, prove the following statements:
\begin{enumerate}
	\item For any $\lambda \in \field{C}$, prove that the determinant of $\boldsymbol{A}- \lambda \boldsymbol{I}_d$ is a polynomial of (at most) degree $d$ in $\lambda$.  We denote it by $p_{\boldsymbol{A}}(\lambda)$ and call it the \emph{characteristic polynomial} of $\boldsymbol{A}$.  We call its roots the \emph{eigenvalues} of $\boldsymbol{A}$.
...
$f$ is continuous in $D \subset \field{R}^d$ if $f$ is continuous at all
points $\x \in D$.
...
\begin{problem}\label{problem:CoerciveFunction}
A continuous real-valued function $f\colon \field{R}^d \to \field{R}$ is said to be \emph{coercive} if the values of $f(\x)$ cannot remain bounded on any non-bounded set $A\subset \field{R}^d$: For all $M>0$ there exists $R=R(M)>0$ so that $f(x)\geq M$ if $\norm{x}\geq R$.  
