...
\bigskip
{\problem[20 pts] \em Find the absolute extrema of $f(x) = 6x^{4/3} - 3x^{1/3}$ on the interval $[-1,1]$.}
\vspace{18cm}
\begin{flushright}
  \begin{tikzpicture}
...
\bigskip
{\problem[30 pts] \em A box with square base is wider than it is tall.
  In order to send the box through the U.S.~mail, the width of the box
  and the perimeter of one of the (nonsquare) sides of the box can sum
  no more than 108~in.  What is the maximum volume for such a box?} 
\vspace{19cm}
\begin{flushright}
  \begin{tikzpicture}
    \draw (-2cm,0.5cm) node {Maximum volume: };
    \draw (0cm,-0.2cm) rectangle (5cm,1.2cm);
  \end{tikzpicture}
\end{flushright}
\end{document}
