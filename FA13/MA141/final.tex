...
\end{flushright}
\hrule
{\problem[15 pts] \em Find the area of the largest rectangle that can
...
\vspace{3cm}
\item $\displaystyle{\int (3x-2)^{200}\, dx}$
\end{enumerate}
\newpage
...
\begin{enumerate}
\item $\displaystyle{g(x) = \int_1^x \frac{1}{t^4+1}\, dt}$
\vspace{3cm}
\begin{flushright}
  \begin{tikzpicture}
 ...
 \end{tikzpicture}
\end{flushright}
\item $\displaystyle{g(x) = \int_{\sin x}^\pi \sqrt{e^t+ t^8}\, dt}$
\vspace{3cm}
\begin{flushright}
  \begin{tikzpicture}
    \draw (-1cm,2.5cm) node {$g'(x) =$};
